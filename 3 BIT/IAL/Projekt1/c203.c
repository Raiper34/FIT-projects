...
**
**    xxxnextIndex ..... pomocná funkce - viz popis vý¹e
**    xxxqueueInit ..... inicializace fronty
**    xxxqueueEmpty .... test na prázdnost fronty
**    xxxqueueFull ..... test, zda je fronta zaplnìna (vyèerpána kapacita)
**    xxxqueueFront .... pøeète hodnoty prvního prvku z fronty
**    xxxqueueRemove ... odstraní první prvek fronty
...
*/
	if(q == NULL) //parameter NULL chyba
	{
		queueError(QERR_INIT);
	}
	else //spravny parameter
	{
		for(int i = 0; i < QUEUE_SIZE; i++) //plni pole znakom *
		{
			q->arr[i] = '*';
		}
		q->f_index = 0;
...
*/
	return((++index) % (QUEUE_SIZE)); //posunie sa o jedno a ak je na konci tak prejde na zaciatok
...
*/
	return(q->f_index == q->b_index); //ak sa prvy prvok rovna poslednemu
}
